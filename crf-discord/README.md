...
## 役員向け活動報告メールの定型(2026-08-19確定)

毎週の役員向け活動報告メールは、以下を標準構成とする。

1. 冒頭: その週のいちばん大きな動き(ニュース)
2. 各トピック(■見出し): Webサイト、Discordのアップデート、カンファレンス、理事会・役員会の議題 など
3. 今後やるべきこと(事務局の認識)
4. **末尾に必ず「■ 今後のスケジュール」を入れる**(これをデフォルトとする)
   - 【定例】読書会(ベルクソン=毎月第2火曜/宮沢賢治=月1回)、信原ゼミ(第4水曜)、発表ゼミ(第2水曜)
   - 【月別】イベント・合宿・フィールドワーク・**森の使用スケジュール**・**役員会/理事会の日程**
   - 末尾に2つのカレンダー(CRF 全イベント / CRF 森予定)のリンク
   - 予定はGoogleカレンダー2つ(`CRF 全イベント` / `CRF 森予定`)から取得して作成する

送信元は info@technel.world(nswtmk@ のエイリアスで切り替え)。送信前に七沢さんの確認を得る。
